...
class DatabasePartnerModule
{
    ...
    public function invitePartner(&$dbID, &$inviteUserID)
    {
        ...
        if ($dbDao->checkDatabasePermission($dbID, $_SESSION['userID'])) {
            ...
            $msg = '<p>您好！亲爱的用户：</p><p>您已经被加入数据库：<b style="color:#4caf50">' . $dbInfo['databaseInfo']['databaseName'] . '</b>，现在你可以参与数据字典的开发协作工作。</p>';
    ...
    public function removePartner($dbID, $connID)
    {
        ...
        if ($dbDao->checkDatabasePermission($dbID, $_SESSION['userID'])) {
            ...
            $msg = '<p>您好！亲爱的用户：</p><p>您已经被移除出数据库：<b style="color:#4caf50">' . $dbInfo['databaseInfo']['databaseName'] . '</b>。</p>';
