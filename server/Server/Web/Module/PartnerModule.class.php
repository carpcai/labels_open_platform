...
/**
 * @name eolinker ams open source，ez.express开放平台
...
class PartnerModule
{
    ...
    public function invitePartner(&$projectID, &$inviteUserID)
    {
        ...
        if ($projectDao->checkProjectPermission($projectID, $_SESSION['userID'])) {
            ...
            $msg = '<p>您好！亲爱的用户：</p><p>您已经被加入项目：<b style="color:#4caf50">' . $projectInfo['projectName'] . '</b>，请妥善保管好以下信息</p>
                <p>App Key：<b style="color:#4caf50"></b></p>
                <p>App Secret：<b style="color:#4caf50"></b></p>';
    ...
    public function removePartner($projectID, $connID)
    {
        ...
        if ($projectDao->checkProjectPermission($projectID, $_SESSION['userID'])) {
            ...
            $msg = '<p>您好！亲爱的用户：</p><p>您已经被移除出项目：<b style="color:#4caf50">' . $projectInfo['projectName'] . '</b>。</p>';
